...
> ⚠️ Note: The BEV visualizations may appear denser after degradation due to fixed rendering sampling, not actual point increase.
...
Supplementary videos demonstrating the LiDAR augmentation framework are available below:

👉 **[Watch Supplementary Videos](https://drive.google.com/file/d/1wkzbbM6RrCp_E2YfEkm33TEMSjfJS7Jq/view?usp=sharing)**

### 📌 What the videos show

- **Real-time LiDAR augmentation** under different severity levels (light → extreme)
- **Original vs augmented point clouds**:
  - Green: original point cloud
  - Red: degraded (augmented) point cloud
- **Bird’s-Eye-View (BEV) visualization** for qualitative inspection
- **Monitoring dashboard** displaying:
  - Processing time per frame
  - Point count reduction
  - Active augmentation parameters

### ⚠️ Visualization Notes

- The BEV view accumulates point clouds before rendering and is therefore less dynamic than the dashboard.
- A fixed number of points is rendered for visualization, which may cause augmented clouds to appear visually denser despite having fewer points.
- This effect is due to field-of-view reduction and sampling, not an increase in actual point density.

### 🎯 Purpose

The videos are intended to illustrate:
- The **visual impact of LiDAR degradations**
- The **real-time behavior of the augmentation pipeline**
- The interaction between augmentation, visualization, and monitoring tools

For full-resolution visualizations and additional screenshots, please refer to the `results/images/` directory.
